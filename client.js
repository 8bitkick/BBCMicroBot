...
const emulationDuration   = 3; //33; // seconds
const startFrame          = 0; //1587; // frames (50fps when vsync active)
...
function (emulator) {
  ...
  async function run(tweet){
    ...
    var start   = new Date()
    var frames  = await emulator.emulate(tweet.text,path,emulationDuration,startFrame);
  ...
  function requestTweet() {
    ...
    options.uri="https://"+HOST+":"+PORT+"/pop";
    options.method="GET";
