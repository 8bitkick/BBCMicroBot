...
This guide will outline how BBC Micro Bot works on Twitter and tricks to reduce the size of your code to fit in a tweet. 
...
The bot will reply to any mention that starts with a line number, contains an `=` sign or contains special characters. This needs improving. If the tweet contains any bad words the tweet is also ignored and the user account blocked. This is automatic and on quite a strict filter.
